...
const DomainForm = ({
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className={clsx('flex gap-4', isEdit ? 'flex-col' : 'items-end')}>
        <FormField
          control={form.control}
          name='name'
          render={({ field }) => (
            <FormItem className={clsx(!isEdit && 'max-h-[72px] w-full sm:w-96')}>
        ...
        <div className='flex items-center gap-2'>
          {validEditableAction && (
            <Button type='button' variant='outline' size='sm' onClick={closeEditIntent}>
              Cancel
            </Button>
          )}
          <Button
